...
contract NFTlottery is Ownable{
    ...
    function createLottery(uint _start, uint _end, address[] memory _participants, uint[] memory _tokens)
        public
        onlyOwner
    {
    ...
    function addParticipant(uint _lottery, address[] memory _toadd)
        public
        onlyOwner
    {    
    ...
    function setWinner(uint _lottery)
        public
        onlyOwner
    {     
         lottery storage L = Lotteries[_lottery];
    ...
    function transferNFT(uint id, address a)
        internal
    {
        Digitible.transferFrom(address(this), a, id);
    }
    
    function random(uint modulus)
        internal
        returns(uint)
    {
    ...
    function _burnParticipant(uint index,uint _lottery)
        internal
    {
